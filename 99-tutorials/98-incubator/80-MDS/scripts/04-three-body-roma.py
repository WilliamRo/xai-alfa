...
class NanoBox(DaVinci, XNode):
  ...
  def __init__(self):
    ...
    self.state_machine._library['h'] = lambda: self.tune_fps(-1)
    self.state_machine._library['l'] = lambda: self.tune_fps(1)
    self.state_machine._library['H'] = lambda: self.tune_fps(-10)
    self.state_machine._library['L'] = lambda: self.tune_fps(10)
    self.state_machine._library['q'] = self.safe_quit

    # Player variables
    self.t: Optional[XNode] = None
    self.fps = 10
    self._tic = None
  ...
  def play(self):
    ...
    if self.t is None:
      self.t = self.execute_a_new_child(_play, long_live=True)
    else:
      self.t.terminate()
      # self.t.thread.join()
      self.t = None
